...
class Node:
	...
	def calc_entropy(self, my_words_idx):
		n = len(my_words_idx)
		p = np.full(n, 1/n)
	...
	def get_entropy(self, all_words, my_words_idx):
		init_entropy = self.calc_entropy(my_words_idx)
		max_id = 0
		info_gain_arr = []
		for word_idx in my_words_idx:
			...
			entropy = -np.sum((value/len(my_words_idx))*self.calc_entropy1(value) for value in split_dict.values())
